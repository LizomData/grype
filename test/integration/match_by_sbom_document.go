...
func RunSBOMTest(sbomPath string) ([]string, []match.Detail, error) {
	vp := newMockDbProvider()
	matches, _, _, err := grype.FindVulnerabilities(vp, "sbom:"+sbomPath, source.SquashedScope, nil)
	if err != nil {
		return nil, nil, err
	}
